...
## Build Dataset 
print(f'Build Source Domain Dataset : {args.mimic_data_dir} ....')
...
if args.mode == "train":
    ...
    print(f'Build Target Domain Dataset : {args.eicu_data_dir} ....')
        ...
        for num_iter, (s_batch_data,t_batch_data) in enumerate((zip(loader_s_train, (loader_t_train)))):
            ...
            if num_iter % args.printiter == 0:
                print("\nTRAIN: EPOCH %04d / %04d | ITER %04d / %04d | LOSS %.4f | err_s_label: %.4f, err_s_domain: %.4f, err_t_domain: %.4f | Source Train Precision : %.4f\n" \
                    ...
                    err_s_domain.detach().cpu().item(), err_t_domain.detach().cpu().item(), precision_train/(num_iter+1)))
        ...
        with torch.no_grad():
            ...
            if num_iter % args.printiter == 0:
                print("\VALID: EPOCH %04d / %04d | ITER %04d / %04d | LOSS %.4f | Source Test Precision : %.4f\n" \
                    % (epoch, args.num_epoch, num_iter+1, len(loader_s_val), err_s_label.detach().cpu().item(), precision_valid/(num_iter + 1)))
