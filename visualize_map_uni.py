...
# Others
parser.add_argument("--ckpt_dir", default="./checkpoint/final_uni_focal", type=str, dest="ckpt_dir")
parser.add_argument("--result_dir", default="./result/final_uni_focal", type=str, dest="result_dir")
...
with torch.no_grad():
    ...
    for idx, batch_data in enumerate(tqdm(dataloader_vis)):
        ...
        class_output, attn_map, latent_vector = model(X_cat,X_num,True) # atttnmap -> (depth,b,head,seq,seq)
...
plt.title(f'Feature Importance - Case {args.label_type}', fontsize=18, fontweight='bold')
plt.xticks(ticks=range(10), labels=top_feature_names, rotation=45, ha="right", fontsize=10)  # x축 레이블 개선
