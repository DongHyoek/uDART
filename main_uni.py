...
# Data
parser.add_argument("--hirid_data_dir", default="./dataset/HiRID_ARDS_12H_SPLIT.csv.gz", type=str, dest="hirid_data_dir")
parser.add_argument("--pic_data_dir", default="./dataset/PICU_ARDS_12H_SPLIT.csv.gz", type=str, dest="pic_data_dir")
...
if args.mode == 'test':
    ...
    dataset_t_train = TableDataset(data_path=args.pic_data_dir, data_type='pic',mode='t_train',seed=args.seed)
    loader_t_train = DataLoader(dataset_t_train, batch_size=args.batch_size, shuffle=True, num_workers=4)

    dataset_t_test = TableDataset(data_path=args.pic_data_dir, data_type='pic',mode='t_test',seed=args.seed)
...
### Inference(Test) mode : Target Test Accuracy ###
    ...
    with torch.no_grad():
        ...
        for num_iter, batch_data in enumerate(tqdm(loader_t_test)):
            ...
            pos_pred_list += list(F.softmax(class_output).detach().cpu().numpy()[:,1])
